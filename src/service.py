...
<section id="view-channels" class="view active"><div class="panel"><div class="section-head"><div><h2>Configured / Remote Channels</h2><div class="hint">Channels load automatically from REMOTE_CONFIG_URL / CHANNELS_URL, channels.json, or CHANNELS_JSON.</div></div><button class="btn" onclick="loadChannels(true)">Refresh</button></div><div id="channelGrid" class="grid"><div class="empty">Loading channels...</div></div></div></section>
...
async function loadChannels(force=false){try{let refreshData=null;if(force){const refresh=await fetch('/api/config/refresh',{method:'POST'});refreshData=await refresh.json();if(!refresh.ok||!refreshData.success){log('Remote catalog refresh failed: '+(refreshData.detail||'unknown error'),'error');}}const r=await fetch('/api/channels',{cache:'no-store'});const data=await r.json();const list=data.channels||[];document.getElementById('channelCount').innerText=list.length;const grid=document.getElementById('channelGrid');if(!list.length){grid.innerHTML='<div class="empty">No configured channels. Set REMOTE_CONFIG_URL, CHANNELS_URL, CHANNELS_CONFIG_URL, or CHANNELS_JSON.</div>';if(refreshData.lastError)log('Config error: '+refreshData.lastError,'error');return;}
...
loadChannels(false);setInterval(()=>loadChannels(false),15000);
</script>
</body></html>
"""
